...
/**
 * Created by jahhein on 6/16/15 at 1:16 AM. Project: codingbat. * Copyright © 2015-2016. Jacob Alan Hein. All Rights
 * Reserved. Access to this file is strictly confidential. This source code may not be copied and/or distributed without
 * the express permission of Jacob Alan Hein. * THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS
 ...
 * SOFTWARE OR THE USE OR OTHER DEALINGS IN THE SOFTWARE. * Contact: Jah.hein.personal@gmail.com (OR)
 * Heinja2@miamioh.edu
 *
 * @author Jacob Hein
 */
 ...
 */
  ...
  public String stringBits(String str) {
    return str.length() <= 1 ? (str.length() == 0 ? "" : "" + str.charAt(0))
  ...
  public String stringSplosion(String str) {
    ...
    while (l-- >= 2) result += str.substring(0, cnt++);
    return result;
  }
  ...
  public int last2(String str) {
    ...
    for (int u = 0; u < str.length() - 2; u++)
      if (str.substring(u, u + 2).equals(str.substring(str.length() - 2))) cnt++;
    return cnt;
  }
  ...
  public boolean array123(int[] nums) {
    if (nums.length >= 3)
      for (int u = 0; u < nums.length - 2; u++)
        if (nums[u] == 1 && nums[u + 1] == 2 && nums[u + 2] == 3) return true;
    return false;
  }
  ...
  public String stringX2(String str) {
    ...
    for (int u = 1; u < str.length() - 1; u++)
      if (str.charAt(u) != 'x') result += str.charAt(u);
  ...
  public String altPairs(String str) {
    ...
    for (int u = 0; u < str.length(); u += 4) {
      result += str.charAt(u);
      if (u + 1 < str.length()) result += str.charAt(u + 1);
    }
    return result;
  }
  ...
  public String stringYak(String str) {
    while (str.contains("yak")) str = str.substring(0, str.indexOf("yak")) + str.substring(str.indexOf("yak") + 3);
    return str;
  }
  ...
  public int array667(int[] nums) {
    ...
    for (int u = 0; u < nums.length - 1; u++)
      if (nums[u] == 6 && (nums[u + 1] == 6 || nums[u + 1] == 7)) result++;
    return result;
  }
  ...
  public boolean noTriples(int[] nums) {
    if (nums.length >= 3)
      for (int u = 0; u < nums.length - 3; u++)
        if (nums[u] == nums[u + 1] && nums[u + 1] == nums[u + 2]) return false;
  ...
  public boolean has271(int[] nums) {
    for (int u = 0; u < nums.length - 2; u++)
      if (nums[u] == nums[u + 1] - 5 && Math.abs(nums[u + 2] + 1 - nums[u]) <= 2) return true;
    return false;
  }
}
